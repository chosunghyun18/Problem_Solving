...
public class Programmers0703 {
    ...
    public int solution2(int[][] maps) {
        ...
        if(maps[0][1] == 0 && maps[1][0] == 0 ) return  -1 ;
        ...
        if(visit[y-1][x-1]) return  cost[y-1][x-1];
        return -1 ;

    }
    public boolean check(int[][] maps ,boolean[][] visit ,int maxX ,int maxY , int currX , int currY) {
        if( maxX <= currX || currX <=-1 || maxY <= currY || currY <=-1 ){
            return false ;
        }
        if(maps[currY][currX] == 0)return true;

        if(visit[currY][currX])return false;
    ...
    public int solution(int[][] maps) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < x && ny >= 0 && ny < y && maps[ny][nx] == 1 && cost[ny][nx] == -1) {
                    cost[ny][nx] = currCost + 1;
